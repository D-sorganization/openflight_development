...
def estimate_launch_angle(
    ...
    _OPTIMAL_SMASH = {
        ...
        ClubType.IRON_5: 1.34,
        ClubType.IRON_6: 1.33,
...
def test_estimate_passes_club_speed(self):
    ...
    angle_with_smash, conf = estimate_launch_angle(ClubType.DRIVER, 143, club_speed_mph=96.6)
...
def test_estimate_passes_spin(self):
    ...
    angle, conf = estimate_launch_angle(ClubType.DRIVER, 143, club_speed_mph=96.6, spin_rpm=2500)
    assert conf == 0.5
```
...
if shot.launch_angle_vertical is None and shot.mode != "mock":
    ...
    logger.info("Estimated launch angle: %.1f° (conf: %.0f%%)", estimated[0], estimated[1] * 100)
```
