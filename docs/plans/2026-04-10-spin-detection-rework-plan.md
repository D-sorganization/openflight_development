...
def detect_spin(
    ...
    if fft_snr < self.SPIN_SNR_MIN and not autocorr_confirmed:
        return SpinResult.no_spin_detected(f"SNR too low ({fft_snr:.1f}, need {self.SPIN_SNR_MIN})")
...
```python
# Try spin detection
ball_speeds = self.extract_ball_speeds(timeline, ball_timestamp_ms, ball_speed_mph)
...
for c in captures:
    ...
    if spin.spin_rpm > 0:
        ...
        club = shot.get("club", "?")
        print(
            f"  Shot {c['shot_number']} ({club}, {ball_speed:.0f}mph): "
