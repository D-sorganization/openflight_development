...
def print_tdat_summary(frames):
    ...
    print(
        f"  Angle:             {min(angles):.1f}\u00b0 to {max(angles):.1f}\u00b0  (mean {sum(angles) / len(angles):.1f}\u00b0)"
    )
    print(
        f"  Distance:          {min(distances):.2f}m to {max(distances):.2f}m  (mean {sum(distances) / len(distances):.2f}m)"
    )
    print(
        f"  Speed:             {min(speeds):.1f} to {max(speeds):.1f} km/h  (WARNING: aliased at golf speeds)"
    )
    print(
        f"  Magnitude:         {min(magnitudes):.0f} to {max(magnitudes):.0f}  (mean {sum(magnitudes) / len(magnitudes):.0f})"
    )
...
def print_timeline(frames, detections_only=False, show_pdat=False):
    ...
    if show_pdat:
        print(
            f"  {'#':>5s}  {'time(s)':>8s}  {'dist(m)':>8s}  {'speed':>8s}  {'angle':>7s}  {'mag':>5s}  {'src':>4s}"
        )
        print(
            f"  {'-----':>5s}  {'--------':>8s}  {'--------':>8s}  {'--------':>8s}  {'-------':>7s}  {'-----':>5s}  {'----':>4s}"
        )
    else:
        print(
            f"  {'#':>5s}  {'time(s)':>8s}  {'dist(m)':>8s}  {'speed':>8s}  {'angle':>7s}  {'mag':>5s}"
        )
        print(
            f"  {'-----':>5s}  {'--------':>8s}  {'--------':>8s}  {'--------':>8s}  {'-------':>7s}  {'-----':>5s}"
        )
...
def print_events(frames, events):
    ...
    print("=" * 60)
    print(
        f"  {'Event':>5s}  {'time(s)':>8s}  {'frames':>6s}  {'dur(ms)':>8s}  {'angle':>7s}  {'dist(m)':>8s}  {'mag':>5s}"
    )
    print(
        f"  {'-----':>5s}  {'--------':>8s}  {'------':>6s}  {'--------':>8s}  {'-------':>7s}  {'--------':>8s}  {'-----':>5s}"
    )
...
def plot_capture(frames, output_path):
    ...
    if tdat_t:
        ax.scatter(
            tdat_t, tdat_angle, s=20, alpha=0.8, label="TDAT", color="tab:orange", marker="x"
        )
    ax.set_ylabel("Angle (\u00b0)")
...
def main():
    ...
    parser.add_argument(
        "--pdat", action="store_true", help="Show PDAT multi-target details in timeline"
    )
    parser.add_argument(
        "--detections-only", action="store_true", help="Only show frames with detections"
    )
    parser.add_argument("--timeline", action="store_true", help="Show full frame-by-frame timeline")
    parser.add_argument(
        "--pair-shots", action="store_true", help="Show probable club-to-ball shot pairs"
    )
    parser.add_argument(
