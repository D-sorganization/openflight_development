    ...
    if not port:
        ...
        for c in candidates:
            if Path(c).exists():
                ...
                break
    if not port:
        print("[calibrate] No port specified and no /dev/kld7_* symlink found.", file=sys.stderr)
...
def main() -> int:
    ...
    parser.add_argument(
        "--port", default=None, help="Serial port (default: auto-detect /dev/kld7_vertical)"
    )
    ...
    parser.add_argument(
        ...
        choices=[5, 10, 30, 100],
        dest="range_m",
        help="K-LD7 max range setting in meters",
    )
    parser.add_argument(
        "--capture-seconds",
        type=float,
        default=None,
        help="Capture for N seconds, summarize, exit. If omitted: run live monitor until Ctrl-C.",
    )
    parser.add_argument(
        "--label", default="", help="Label written into the saved JSON (e.g. '15deg_5m')"
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help="Skip frames where the dominant peak is too weak (default 3.0× = ~5 dB)",
    )
    parser.add_argument(
        "--print-every",
        type=int,
    ...
    try:
        ...
        print(
            "[calibrate] kld7 package not installed. Reinstall the project: "
            "uv pip install -e '.[ui]'",
            file=sys.stderr,
        )
        return 2
    ...
    if in_warmup:
        print(
            f"[calibrate] Warmup: get the reflector into position. "
        ...
        for code, payload in radar.stream_frames(FrameCode.RADC, max_count=-1):
            ...
            if now < capture_start:
                ...
                if now - last_countdown >= 1.0:
                    ...
                    roll_std = (
                        statistics.pstdev(rolling_angles) if len(rolling_angles) >= 2 else 0.0
                    )
                    print(
                        f"  [warmup {remaining:4.1f}s] angle={r['angle_deg']:+6.2f}°  "
            ...
            if in_warmup:
                ...
                if args.capture_seconds:
                    print(f"\n[calibrate] >>> Capturing now for {args.capture_seconds:.0f}s <<<\n")
                else:
    ...
    high_snr_pairs = [
        (r["angle_deg"], r["snr_linear"]) for r in readings if r["snr_linear"] >= 10.0
    ]
    if high_snr_pairs:
    ...
    if args.output:
        ...
        out_data = {
            ...
            "summary": summary,
            "high_snr_summary": (
                summarize(list(hi_angles), list(hi_snrs)) if high_snr_pairs else None
            ),
            "readings": readings,
        }
