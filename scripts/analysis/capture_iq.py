...
Examples:
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Output .pkl file path (default: auto-generated in ~/openflight_sessions)",
    )
    parser.add_argument("--port", help="Serial port for radar (auto-detect if not specified)")
    parser.add_argument(
        "--pre-trigger",
        "-p",
        type=int,
        ...
        while True:
            ...
            if result:
                ...
                if result.spin and result.spin.spin_rpm > 0:
                    ...
                    spin_str = (
                        f"  spin=none ({result.spin.rejection_reason}, snr={result.spin.snr:.1f})"
                    )
            ...
            if result:
                ...
                capture_dict["club_speed_mph"] = (
                    round(result.club_speed_mph, 1) if result.club_speed_mph else None
                )
                capture_dict["smash_factor"] = (
                    round(result.smash_factor, 2) if result.smash_factor else None
                )
                if result.spin:
                    ...
                    capture_dict["spin_candidate_rpm"] = (
                        ...
                        if result.spin.peak_freq_hz is not None
                        else None
                    )
                    ...
                    capture_dict["spin_rejection_reason"] = result.spin.rejection_reason
