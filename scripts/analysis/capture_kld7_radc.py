    ...
    def _read_loop(self):
        ...
        while self._running:
            try:
                ...
                capture_entry["spin_modulation_depth"] = spin.modulation_depth if spin else None
                ...
                capture_entry["spin_candidate_rpm"] = (
                    ...
                    if spin and spin.peak_freq_hz is not None
                    else None
                )
                ...
                capture_entry["spin_rejection_reason"] = spin.rejection_reason if spin else None
                ...
                if spin and spin.spin_rpm > 0:
                    spin_str = (
                        f", spin: {spin.spin_rpm:.0f} rpm (snr={spin.snr:.1f}, {spin.quality})"
                    )
                elif spin and spin.rejection_reason:
                    spin_str = f", spin: none ({spin.rejection_reason}, snr={spin.snr:.1f})"
                print(
...
def _baseline_clutter_report(
    frames: list, fft_size: int = 2048, max_speed_kmh: float = 100.0, top_n: int = 10
) -> None:
    ...
    print(
        f"  {'rank':>4}  {'bin':>5}  {'velocity':>11}  {'med mag':>10}  {'×floor':>7}  {'note':<30}"
    )
    flagged = False
    for rank, b in enumerate(top_indices, 1):
        ...
        print(
            f"  {rank:>4}  {int(b):>5}  {vel:>+8.1f} km/h  {mag:>10.2f}  {ratio:>6.1f}×  {note:<30}"
        )
    print()
    if flagged:
...
def main():
    ...
    parser.add_argument(
        "--baud", type=int, default=3000000, help="K-LD7 baud rate (default: 3000000)"
    )
    ...
    print(
        f"  Targets:     {'PDAT/TDAT included' if args.include_targets else 'disabled (RADC only)'}"
    )
    print()
    ...
    try:
        ...
        print(
            f"  WARN: connect_with_recovery unavailable ({e}); "
        ...
        while time.time() - start_time < args.duration and stream_errors < max_stream_errors:
            try:
                for code, payload in kld7.stream_frames(frame_codes, max_count=-1):
                    ...
                    if code == "RADC" and (not isinstance(payload, bytes) or len(payload) != 3072):
                    ...
                    if code == "RADC":
                        ...
                        current_frame["pdat"] = (
                            [target_to_dict(t) for t in payload] if payload else []
                        )
