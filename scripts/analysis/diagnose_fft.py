...
def analyze_capture(response: str):
    ...
    print("\n=== I/Q Data Stats ===")
    print(
        f"I samples: {len(i_samples)}, range: {i_samples.min()}-{i_samples.max()}, mean: {i_samples.mean():.1f}, std: {np.std(i_samples):.1f}"
    )
    print(
        f"Q samples: {len(q_samples)}, range: {q_samples.min()}-{q_samples.max()}, mean: {q_samples.mean():.1f}, std: {np.std(q_samples):.1f}"
    )
    ...
    print(
        f"POS max in range: bin {pos_golf_max_idx} -> {bin_to_mph(pos_golf_max_idx):.1f} mph, mag {magnitude[pos_golf_max_idx]:.1f}"
    )
    print(
        f"NEG max in range: bin {neg_golf_max_idx - FFT_SIZE} -> {bin_to_mph(neg_golf_max_idx - FFT_SIZE):.1f} mph, mag {magnitude[neg_golf_max_idx]:.1f}"
    )
    print(
        f"Noise floor in golf range: {np.median(pos_golf):.1f} (pos), {np.median(neg_golf):.1f} (neg)"
    )
    ...
    for block_idx in range(min(5, len(i_samples) // 128)):
        ...
        print(
            f"Block {block_idx}: pos_peak bin {pos_peak} ({pos_mph:.1f} mph, mag {mag_w[pos_peak]:.4f}) | "
