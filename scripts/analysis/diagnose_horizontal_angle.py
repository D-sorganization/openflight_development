...
def print_summary(rows: list[ShotRow]) -> None:
    print("=" * 100)
    print(f"DIAGNOSTIC SUMMARY  —  {len(rows)} shots, {len({r.session_id for r in rows})} sessions")
    print("=" * 100)
    ...
    print(
        f"angle_source: radar={len(h_radar)}  estimated={len(h_est)}  "
    ...
    print(
        f"  shots where live ball_angle was returned: {have_live}  "
        f"({have_live * 100 / max(1, have_buf):.1f}% of bufs)"
    )
    miss = have_buf - have_live
    print(
        f"  shots where horizontal radar saw nothing: {miss}  "
    ...
    if h_radar:
        ...
        print(
            f"   |h|>=14°: {wall_close} ({wall_close * 100 / len(absh):.1f}%)   "
            f"|h|>=14.9°: {wall_at}"
        )
    print()
    ...
    for k in sorted(cb):
        xs = cb[k]
        print(
            f"  {k:>6.1f} {len(xs):>5} {stats.mean(xs):>9.2f} "
    ...
    for sid in {r.session_id for r in h_radar}:
        seq = sorted([r for r in h_radar if r.session_id == sid], key=lambda r: r.shot_number)
    ...
    # Face vs path
    pairs = [
        (r.h_angle, r.club_path_deg, r.spin_axis_deg)
        for r in rows
    ...
    if pairs:
        ...
        print(
            f"Face vs path (radar): n={len(pairs)}  "
        ...
        if len(xs) >= 5 and stats.pstdev(xs) > 0 and stats.pstdev(ys) > 0:
            ...
            print(
                f"  corr(face, path) = {r:+.3f}  (near 0 = independent peaks; near 1 = same target)"
            )
    print()
    ...
    print(
        f"  {'session':<24}{'n':>4}{'detect%':>9}{'mean':>8}{'stdev':>8}{'med|Δ|':>9}{'p90|Δ|':>9}"
    )
    for sid in sorted({r.session_id for r in rows}):
        ses = [r for r in rows if r.session_id == sid]
        rad = [r for r in ses if r.angle_source == "radar" and r.h_angle is not None]
        if not rad:
            continue
...
def plot_distribution(rows: list[ShotRow], out_dir: Path) -> None:
    ...
    for i, sid in enumerate(sessions):
        ...
        if ds:
            ax.plot(ds, alpha=0.55, color=cmap(i % 10), label=sid.replace("session_", ""))
...
def plot_per_session(rows: list[ShotRow], out_dir: Path) -> None:
    ...
    for sid in sessions:
        ...
        ax2 = ax.twinx()
        snrs = [r.h_avg_snr_db if r.h_avg_snr_db is not None else np.nan for r in ses]
        ...
        cmap = {"radar": "#4CAF50", "estimated": "#9E9E9E", "camera": "#2196F3", "none": "#F44336"}
        ax.bar(
            xs, confs, color=[cmap.get(s, "#F44336") for s in sources], edgecolor="k", linewidth=0.3
        )
...
def plot_face_vs_path_scatter(rows: list[ShotRow], out_dir: Path) -> None:
    pairs = [
        (r.h_angle, r.club_path_deg, r.confidence or 0)
        for r in rows
        if r.h_angle is not None and r.club_path_deg is not None
    ]
    if not pairs:
    ...
    sc = ax.scatter(xs, ys, c=cs, cmap="viridis", s=30, edgecolor="k", linewidth=0.3)
    lim = max(15, max(abs(min(xs + ys)), abs(max(xs + ys))))
    ax.plot([-lim, lim], [-lim, lim], "k--", alpha=0.5, label="face = path (zero spin axis)")
    ax.set_xlim(-lim, lim)
    ax.set_ylim(-lim, lim)
...
def write_csv(rows: list[ShotRow], out_dir: Path) -> None:
    ...
    fields = [
        ...
        "h_num_frames",
        "h_avg_snr_db",
        "h_buffer_frame_count",
        "h_buffer_span_s",
        "v_buffer_frame_count",
        "v_ball_angle",
        "v_confidence",
    ]
...
def group_frames_around_shots(
    capture: dict,
    ms_before: float = 1500.0,
    ms_after: float = 700.0,
) -> list[dict]:
...
def anchored_replay(
    ...
    survivors = [
        b
        for b in breakdown
        if b["peak_snr"] >= snr_floor and abs(b["peak_bin"] - ops_expected_bin) <= ops_bin_tol
    ]
    if not survivors:
        return None
    snrs = np.array([b["peak_snr"] for b in survivors], dtype=float)
...
def replay_capture(
    ...
    if dc_mask_bins is not None and dc_mask_bins != original_dc:
        ...
        radc.compute_spectrum.__defaults__ = (
            radc.compute_spectrum.__defaults__[0],
            dc_mask_bins,
        )
        radc.compute_fft_complex.__defaults__ = (
            radc.compute_fft_complex.__defaults__[0],
            dc_mask_bins,
        )
        print(f"NOTE: DC mask overridden to {dc_mask_bins} bins (default {original_dc})")
    ...
    try:
        ...
        print(f"  ops243 shots in capture = {len(cap.get('ops243_shots') or [])}")
        print(f"  total frames = {md.get('total_frames')}  (with RADC: {md.get('radc_frames')})")
        ...
        for shot_idx, g in enumerate(grouped, start=1):
            ...
            tag = aim_tags[shot_idx - 1] if (aim_tags and shot_idx - 1 < len(aim_tags)) else None
            ...
            summary.append(
                {
                    ...
                    "strict_cluster_bin": (strict["cluster_bin_center"] if strict else None),
                    "strict_cluster_span": (strict["cluster_bin_span"] if strict else None),
                    "strict_frames_above_snr": (strict["frames_above_snr"] if strict else 0),
                    "anchored_angle": (anchored["launch_angle_deg"] if anchored else None),
                    "anchored_frame_count": (anchored["frame_count"] if anchored else 0),
                    "anchored_avg_snr": (anchored["avg_snr_db"] if anchored else None),
                    "anchored_max_snr": (anchored["max_snr_db"] if anchored else None),
                    "anchored_angle_std": (anchored["angle_std_deg"] if anchored else None),
                    "anchored_bin_offset_mean": (anchored["bin_offset_mean"] if anchored else None),
                    "anchored_bin_offset_max": (anchored["bin_offset_max"] if anchored else None),
                    "ops_expected_bin": ops_bin,
                    "band_tol_mph": band_tol_mph,
                    "ops_bin_tol": ops_bin_tol,
        ...
        _write_radc_csv(
            ...
            path,
            out_dir,
            strict_snr=strict_snr,
            anchored=ops_bin_tol is not None,
        )
    finally:
        if dc_mask_bins is not None and dc_mask_bins != original_dc:
...
def _print_radc_summary(summary: list[dict], orientation: str) -> None:
    ...
    print(
        f"  shots with live angle   : {len(detected)}  ({len(detected) * 100 / len(summary):.1f}%)"
    )
    ...
    if detected:
        ...
        print(f"  live confidence         : mean={np.mean(confs):.2f}  min={min(confs):.2f}")
        print(f"  live avg SNR (dB)       : mean={np.mean(snrs):.1f}  min={min(snrs):.1f}")
    ...
    for s in summary:
        ...
        if len(bins) >= 2:
            per_shot_bin_jumps.append(float(np.mean(np.abs(np.diff(bins)))))
            per_shot_angle_jumps.append(float(np.mean(np.abs(np.diff(angs)))))
    if per_shot_bin_jumps:
        print(
            f"  mean |Δ peak_bin| / shot   : "
            f"{np.mean(per_shot_bin_jumps):.1f} bins  "
            f"(small = stable peak; large = jumping target)"
        )
        print(
            f"  mean |Δ peak_angle| / shot : "
...
def _plot_radc_shot(
    ...
    axes[1].axhline(2.0, color="k", linestyle="--", alpha=0.4, label="multi-frame SNR floor (2.0)")
    axes[1].axhline(5.0, color="r", linestyle="--", alpha=0.4, label="single-frame SNR floor (5.0)")
    ...
    if live_result is not None:
        axes[2].axhline(
            live_result["launch_angle_deg"],
            color=C_DIFF,
            linestyle="--",
            alpha=0.8,
            label=(
                f"live angle {live_result['launch_angle_deg']:+.1f}°  "
...
def _ops243_expected_bin(
    ball_speed_mph: float, fft_size: int = 2048, max_speed_kmh: float = 100.0
) -> int:
...
def _plot_radc_shot_spectrum(
    ...
    axes[0].axvspan(fft_size - radc.DC_MASK_BINS, fft_size, color="#F44336", alpha=0.25)
    ...
    title = (
        f"shot {shot_idx} frame {bd['frame_index']} (highest SNR)  —  ball_speed {ball_mph:.1f} mph"
    )
    if tag:
        title += f"  —  aim={tag}"
    axes[0].set_title(title)
...
def _plot_radc_shot_spectrogram(
    ...
    ax.axvspan(
        fft_size - radc.DC_MASK_BINS, fft_size, color="#F44336", alpha=0.20, lw=0, label="DC mask"
    )
    ax.scatter(
        peak_bins,
        [f + 0.5 for f in peak_frames],
        color="#00E5FF",
        s=20,
        edgecolor="k",
        linewidth=0.4,
...
def _plot_radc_aim_vs_angle(
    ...
    for s in tagged:
        ...
        if s["per_frame_angles"]:
            ax.scatter(
                [s["shot_idx"]] * len(s["per_frame_angles"]),
                s["per_frame_angles"],
                color=c,
                alpha=0.25,
                s=18,
    ...
    for s in tagged:
        ...
        if s["per_frame_bins"]:
            ...
            ax.scatter(
                [s["shot_idx"]] * len(s["per_frame_bins"]),
                s["per_frame_bins"],
                color=c,
                alpha=0.3,
                s=18,
            ...
            ax.scatter(
                [s["shot_idx"]], [med], color=c, s=140, edgecolor="k", linewidth=1.0, zorder=5
            )
    ax.set_xlabel("shot index")
...
def _plot_radc_overview(
    ...
    if detected:
        axes[0].hist(
            [s["live_angle"] for s in detected],
    ...
    rate = [d / max(1, w) for d, w in zip(detected_frames, in_win)]
...
def _print_anchored_summary(
    ...
    print(
        f"  band_tol_mph             : +/- {band_tol_mph:.1f} mph "
        f"(narrow ball band around OPS speed)"
    )
    print(f"  ops_bin_tol              : +/- {ops_bin_tol} bins around OPS-expected bin")
    print(f"  shots accepted           : {len(hits)} / {n}  ({len(hits) * 100 / max(1, n):.1f}%)")
    ...
    if hits:
        ...
        print(f"  cluster size             : mean={np.mean(fcs):.1f}  max={max(fcs)}")
        print(f"  cluster avg SNR          : mean={np.mean(snrs):.1f}  min={min(snrs):.1f}")
...
def _plot_anchored_vs_live(summary: list[dict], out_dir: Path) -> None:
    ...
    for s in summary:
        ...
        if s.get("aim_tag") == "L":
            ...
            c = "#C62828"
        if live is not None:
            ax.scatter([x], [live], color=c, marker="o", s=110, edgecolor="k", linewidth=0.8)
        if anch is not None:
            ax.scatter([x], [anch], color=c, marker="s", s=140, edgecolor="k", linewidth=0.8)
        if live is not None and anch is not None:
            ax.plot([x, x], [live, anch], color=c, linestyle=":", alpha=0.6, linewidth=1.2)
    ax.axhline(0, color="k", linewidth=0.5)
    ax.axhline(15, color="#F44336", linestyle="--", alpha=0.4)
    ax.axhline(-15, color="#F44336", linestyle="--", alpha=0.4)
    ax.set_xticks(xs)
    ...
    ax = axes[1]
    live_vals = [s["live_angle"] for s in summary if s["live_angle"] is not None]
    anch_vals = [s["anchored_angle"] for s in summary if s["anchored_angle"] is not None]
    ...
    if anch_vals:
        ax.hist(
            ...
            alpha=0.55,
            edgecolor="k",
            label=f"anchored (n={len(anch_vals)})",
        )
    ax.axvline(0, color="k", linewidth=0.5)
...
def _print_strict_snr_summary(
    ...
    print(f"  cluster_bin_tol         : +/- {cluster_tol} bins, >=2 frames required")
    print(
        f"  shots accepted          : {len(strict_hits)} / {n}  "
    ...
    if strict_hits:
        ...
        print(f"  cluster size            : mean={np.mean(fc):.1f} frames  max={max(fc)}")
        print(
            f"  cluster bin span        : "
...
def _plot_strict_vs_live(summary: list[dict], out_dir: Path) -> None:
    ...
    for s in summary:
        ...
        if live is not None and strict is not None:
            ax.plot(
                [x, x], [live, strict], color=tag_color, linestyle=":", alpha=0.6, linewidth=1.2
            )
        xs.append(x)
    ax.axhline(0, color="k", linewidth=0.5)
    ax.axhline(15, color="#F44336", linestyle="--", alpha=0.4)
    ...
    ax = axes[1]
    live_vals = [s["live_angle"] for s in summary if s["live_angle"] is not None]
    strict_vals = [s["strict_angle"] for s in summary if s["strict_angle"] is not None]
    ...
    if strict_vals:
        ax.hist(
            ...
            alpha=0.55,
            edgecolor="k",
            label=f"strict (n={len(strict_vals)})",
        )
    ax.axvline(0, color="k", linewidth=0.5)
    ax.set_xlabel("angle (deg)")
    ax.set_ylabel("count")
...
def main() -> None:
    ...
    ap.add_argument(
        "logs", nargs="*", type=Path, help="JSONL session log paths (or globs expanded by shell)"
    )
    ap.add_argument(
        "--radc",
        type=Path,
        default=None,
        help="Path to a kld7_radc_*.pkl raw RADC capture; enables full FFT replay diagnostics",
    )
    ap.add_argument(
        "--dc-mask",
        type=int,
    ...
    if args.radc:
        ...
        aim_tags = [t.strip() for t in args.aim.split(",")] if args.aim else None
        replay_capture(
            args.radc,
            args.output_dir,
            dc_mask_bins=args.dc_mask,
            aim_tags=aim_tags,
