...
def _select_phase_peak(
    ...
    if strongest_idx < lower_rail_limit or (
        strongest_error > processor.SPIN_PRIOR_STRONGEST_FAR_ERROR and best_error < strongest_error
    ):
        return best_idx
    return strongest_idx
            ...
            rows.append(
                {
                    ...
                    "phase_seam_cycles": (
                        round(phase.seam_cycles, 2) if phase.seam_cycles is not None else None
                    ),
...
def _print_summary(rows: list[dict[str, Any]]) -> None:
    ...
    for method in sorted({row["method"] for row in rows}):
        ...
        recoveries = {
            ...
            for row in rows
            if row["method"] == method and row["combined_accept"] and not row["envelope_reportable"]
        }
        union_errors = {
            **env_once,
            **recoveries,
    ...
    for name, errors in strategies:
        ...
        print(
            f"{name:<28} {len(errors):>3} {mae:>8.1f} {bias:>8.1f} {rmse:>8.1f}"
            if errors
            else f"{name:<28} {0:>3} {'-':>8} {'-':>8} {'-':>8}"
        )
