...
def _ops243_expected_bin(
    radc, ball_speed_mph: float, fft_size: int = 2048, max_speed_kmh: float = 100.0
) -> int:
...
def render(
    pkl_path: Path, shot_idx: int, out_path: Path, floor_frac: float = 0.5, zoom_pad_bins: int = 80
) -> None:
    radc = _import_radc()
    ...
    centroid_angle = float(np.sum(angles[neigh_indices] * neigh_w) / float(neigh_w.sum()))
    ...
    axes[0].axhline(
        ...
        linewidth=1.2,
        alpha=0.8,
        label=f"floor = {int(floor_frac * 100)}% of peak",
    )
    axes[0].axvline(
        peak_bin,
        color="#C62828",
    ...
    axes[1].axvspan(
        neigh_indices.min(), neigh_indices.max() + 1, color="#FFD54F", alpha=0.55, zorder=1
    )
    axes[1].axvline(peak_bin, color="#C62828", linestyle="-", linewidth=2.0, zorder=3)
    axes[1].scatter(neigh_indices, angles[neigh_indices], s=22, color="#2E7D32", zorder=4)
    ...
    print(
        f"  new centroid angle      : {centroid_angle:+.2f}°  "
...
def render_spectrogram(
    ...
    ax.axvspan(
        ...
        alpha=0.22,
        lw=0,
        label=f"NEW: centroid window for highest-SNR frame (±{radc.CENTROID_SEARCH_BINS} bins)",
    )
    ...
    ax.axhspan(
        best_frame, best_frame + 1, facecolor="none", edgecolor="#FFFFFF", linewidth=1.2, alpha=0.7
    )
    ax.text(z_lo + 2, best_frame + 0.5, "highest SNR", color="white", fontsize=8, va="center")
    ...
    if per_frame_peak:
        ...
        ax.scatter(
            ...
            edgecolor="white",
            linewidth=0.6,
            label="OLD: per-frame single-bin peak",
            zorder=4,
        )
    ...
    if per_frame_centroid:
        ...
        ax.scatter(
            ...
            edgecolor="white",
            linewidth=0.6,
            label="NEW: per-frame centroid bin",
            zorder=5,
        )
    ...
    ax.set_title(
        f"shot {shot_idx} spectrogram — ball streak across frames  (ball {ball_mph:.1f} mph)"
    )
...
def main() -> int:
    ...
    parser.add_argument("--pkl", required=True, type=Path, help="Capture pickle (kld7_radc_*.pkl)")
    parser.add_argument(
        "--shot", type=int, default=3, help="1-based shot index in the capture (default 3)"
    )
    parser.add_argument(
        "--out",
        type=Path,
    ...
    parser.add_argument(
        "--mode",
        choices=["spectrum", "spectrogram", "both"],
        default="spectrum",
    ...
    if args.mode in ("spectrum", "both"):
        ...
        render(
            args.pkl, args.shot, spec_out, floor_frac=args.floor_frac, zoom_pad_bins=args.zoom_pad
        )
    if args.mode in ("spectrogram", "both"):
        ...
        render_spectrogram(
            args.pkl, args.shot, sgram_out, floor_frac=args.floor_frac, zoom_pad_bins=args.zoom_pad
        )
    return 0
