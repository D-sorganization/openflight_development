...
def _gather_metric(rows: List[Dict], stem: str) -> Tuple[np.ndarray, np.ndarray, List[str]]:
...
def _scatter(ax, of: np.ndarray, tm: np.ndarray, clubs: List[str], label: str, unit: str) -> None:
...
def _bland_altman(
    ax, of: np.ndarray, tm: np.ndarray, clubs: List[str], label: str, unit: str
) -> None:
    ...
    ax.axhline(
        bias, color="#d32f2f", linestyle="-", linewidth=1.2, label=f"bias = {bias:+.2f} {unit}"
    )
    ax.axhline(
        upper, color="#d32f2f", linestyle="--", linewidth=0.8, label=f"+1.96σ = {upper:+.2f}"
    )
    ax.axhline(
        lower, color="#d32f2f", linestyle="--", linewidth=0.8, label=f"-1.96σ = {lower:+.2f}"
    )
...
def plot_metric(
    rows: List[Dict], stem: str, label: str, unit: str, output_dir: Path
) -> Optional[Path]:
...
def _classify_metric(stem: str, deltas: List[float], total_pairs: int) -> Tuple[str, str]:
    ...
    if total_pairs > 0 and n < 0.5 * total_pairs:
        ...
        return "thin", (
            f"only {n}/{total_pairs} shots had a value "
            f"({coverage:.0f}% coverage — most shots had no reading)"
        )
    bias = float(np.mean(deltas))
    sd = float(np.std(deltas, ddof=0))
    good_sd, good_bias, noisy_sd = _TAKEAWAY_RULES.get(stem, (1e9, 1e9, 1e9))
    ...
    if sd > noisy_sd:
        ...
        if abs(bias) > 2 * sd:
            return "noisy", (f"bias {bias:+.1f} but per-shot σ is {sd:.1f} — mostly noise")
        return "noisy", (
            f"per-shot σ is {sd:.1f} — readings vary a lot, can't trust individual shots"
        )
    # Else: biased — bias dominates the noise.
    direction = "low" if bias < 0 else "high"
    return "biased", (
        f"reads {abs(bias):.1f} {direction} on average (σ={sd:.1f} — clean miscalibration)"
    )


def _per_club_summary(
    rows: List[Dict], stem: str
    ...
    return (
        ...
        [
            float(np.std(deltas_by_club[c], ddof=0)) if len(deltas_by_club[c]) > 1 else 0.0
            for c in names
        ],
...
def plot_takeaways(rows: List[Dict], output_dir: Path) -> Path:
    ...
    header.text(
        0.0,
        0.65,
        "OpenFlight vs Trackman — Key Takeaways",
        fontsize=22,
        fontweight="bold",
        color="#222",
    )
    ...
    for idx, (stem, label, unit) in enumerate(_METRICS):
        ...
        if len(deltas) >= 1:
            ...
            num_line = (
                f"mean Δ (OF−TM) = {bias:+.2f} {unit}    σ = {sd:.2f} {unit}    n = {len(deltas)}"
            )
        else:
        ...
        bar_ax.bar(
            ...
            means,
            yerr=sds,
            color=[_color_for_club(c) for c in names],
            edgecolor="white",
            linewidth=0.6,
            capsize=4,
        )
        ...
        bar_ax.set_xticks(x)
        bar_ax.set_xticklabels([f"{n}\nn={ni}" for n, ni in zip(names, ns)], fontsize=8)
...
def plot_overview(rows: List[Dict], output_dir: Path) -> Path:
    ...
    for idx, (stem, label, unit) in enumerate(_METRICS):
        ...
        x = np.arange(len(names))
        ax.bar(x, means, yerr=stds, color=colors, edgecolor="white", linewidth=0.8, capsize=5)
        ax.axhline(0, color="#444", linewidth=0.7)
        ax.set_xticks(x)
        ax.set_xticklabels([f"{n}\n(n={ni})" for n, ni in zip(names, ns)], fontsize=9)
        ax.set_ylabel(f"OF − TM ({unit})")
        ax.set_title(f"{label}: bias ± 1σ per club")
        ax.grid(True, alpha=0.3, axis="y")

    fig.suptitle("OpenFlight vs Trackman — bias overview by club", fontsize=14, y=1.00)
    fig.tight_layout()
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate scatter + Bland-Altman plots from a compare_trackman comparison CSV.",
    )
    parser.add_argument(
        "comparison_csv", type=Path, help="Comparison CSV (output of compare_trackman.py)"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    if not args.comparison_csv.exists():
        print(f"Comparison CSV not found: {args.comparison_csv}", file=sys.stderr)
        return 2

    output_dir = args.output_dir or args.comparison_csv.with_suffix("").parent / (
