...
def plot_radar_card(shot: dict, output: Path):
    ...
    for i, (label, value, color) in enumerate(metrics):
        ...
        ax.text(
            2.0, row, value, fontsize=16, ha="left", va="center", fontweight="bold", color=color
        )
...
def plot_club_metrics(shots: list[dict], output: Path):
    ...
    ax1.bar(
        shot_nums,
        [a if a is not None else 0 for a in aoa],
        color=[C_CLUB if a is not None else "#ccc" for a in aoa],
        alpha=0.85,
    )
    ...
    for i, a in enumerate(aoa):
        if a is not None:
            ax1.text(
                shot_nums[i], a + (0.3 if a >= 0 else -0.8), f"{a:+.1f}°", ha="center", fontsize=10
            )
        else:
    ...
    ax2.bar(
        shot_nums,
        [p if p is not None else 0 for p in path],
        color=[C_HORIZ if p is not None else "#ccc" for p in path],
        alpha=0.85,
    )
    ax2.axhline(y=0, color="#333", linewidth=0.8, linestyle="--")
    for i, p in enumerate(path):
        if p is not None:
            ax2.text(
                shot_nums[i], p + (0.3 if p >= 0 else -1.0), f"{p:+.1f}°", ha="center", fontsize=10
            )
        else:
            ax2.text(shot_nums[i], 0.3, "—", ha="center", fontsize=10, color="#999")
    ax2.set_xlabel("Shot")
...
def plot_overhead(shot: dict, output: Path):
    ...
    if c_path is not None:
        ...
        ax.text(
            0,
            -3.5,
            f"Spin Axis: {spin_axis:+.1f}° ({shape})",
            fontsize=12,
            ha="center",
            fontweight="bold",
            color=C_SPIN,
        ...
        for j, k in enumerate(metric_keys):
            if k is None:
                has = s.get("spin_axis_deg") is not None or (
                    s.get("launch_angle_horizontal") is not None
                    and s.get("club_path_deg") is not None
                )
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory (default: alongside session file)",
    )
    parser.add_argument(
        "--shot",
        type=int,
        default=None,
        help="Generate radar card + overhead for a specific shot number",
    )
