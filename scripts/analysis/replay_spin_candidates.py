...
def _scoreboard_rows(
    ...
    for shot_entry, capture_entry in zip(shots, captures):
        ...
        expected_spin = (
            get_optimal_spin_for_ball_speed(result.ball_speed_mph, club) if result else None
        )
        ...
        for candidate in candidates:
            rows.append(
                _row(
                    ...
                    expected_spin=expected_spin,
                    candidate=candidate,
                    candidate_error=(candidate.rpm - tm_spin if tm_spin is not None else None),
                )
            )
...
def _row(
    ...
    return {
        ...
        "candidate_expected_error_pct": (
            ...
            if candidate and candidate.expected_spin_error_pct is not None
            else None
        ),
        ...
        "candidate_error_rpm": (round(candidate_error, 1) if candidate_error is not None else None),
    }
...
def _print_summary(rows: list[dict[str, Any]]) -> None:
    ...
    reported = [row for row in selected_trackman if row["selected_spin_rpm"] is not None]
...
def main() -> None:
    ...
    if len(shots) != len(captures):
        print(
            f"Warning: {len(shots)} shot entries but {len(captures)} captures; pairing by order",
            file=sys.stderr,
        )
