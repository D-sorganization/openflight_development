...
Examples:
    ...
    parser.add_argument(
        "--no-fft", action="store_true", help="Skip raw FFT frames (smaller output, faster)"
    )
    parser.add_argument(
        "-o", "--output", help="Output .pkl file (default: auto in ~/openflight_sessions/)"
    )
    parser.add_argument(
        "-n",
        "--max-frames",
    ...
    parser.add_argument(
        "--notes", type=str, default=None, help="Session notes (e.g. 'hitting into net at 3m')"
    )
    ...
    print()
    print(
        f"  {'#':>5s}  {'dist(m)':>8s}  {'speed(km/h)':>12s}  {'angle':>7s}  {'mag':>5s}  {'targets':>7s}"
    )
    print(
        f"  {'-----':>5s}  {'--------':>8s}  {'------------':>12s}  {'-------':>7s}  {'-----':>5s}  {'-------':>7s}"
    )
    ...
    try:
        ...
        if frame_count > 0:
            ...
            if pdat_with_data:
                ...
                print(
                    f"  PDAT angle range:  {min(pdat_angles):.1f}\u00b0 to {max(pdat_angles):.1f}\u00b0"
                )
