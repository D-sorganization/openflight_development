...
def process_capture(
    ...
    for reading in timeline.readings:
        ...
        print(
            f"    Seg {reading.segment_index:>2}: {reading.speed_mph:>6.1f} mph "
    ...
    print(
        f"  Peak outbound:  {peak_reading.speed_mph:.1f} mph "
...
def run(port: str, gpio_pin: int, pre_trigger_segments: int, swings_only: bool, debounce_ms: int):
    ...
    print()
    print(
        f"Pre-trigger: S#{pre_trigger_segments} "
    ...
    try:
        while True:
            ...
            is_swing = process_capture(
                processor, response, trigger_count, transfer_ms, pre_trigger_segments, swings_only
            )
