...
def main():
    parser = argparse.ArgumentParser(description="Direct hardware sound trigger test")
    parser.add_argument(
        "--pre-trigger",
        "-p",
        type=int,
    ...
    print("  Trigger: SEN-14262 → Level Shifter → HOST_INT")
    ...
    try:
        while True:
            print(
                f"[{trigger_count + 1}] Waiting for hardware trigger (timeout={args.timeout}s)..."
            )
            ...
            if response and '"I"' in response and '"Q"' in response:
                ...
                if capture:
                    ...
                    print(
                        f"  Outbound: {len(outbound)} (peak: {max((r.speed_mph for r in outbound), default=0):.1f} mph)"
                    )
                    print(
                        f"  Inbound: {len(inbound)} (peak: {max((r.speed_mph for r in inbound), default=0):.1f} mph)"
                    )
                    ...
                    if outbound_fast:
                        ...
                        print(
                            "  NO SWING: No outbound readings >= 15 mph (false trigger from sound)"
                        )
                else:
