...
def main():
    ...
    # Set up GPIO input
    print(
        f"Setting up GPIO{args.gpio_input} (physical pin {gpio_to_physical(args.gpio_input)}) as input..."
    )
    ...
    print(
        f"  GPIO{args.gpio_input} (physical pin {gpio_to_physical(args.gpio_input)}) ← SEN-14262 GATE"
    )
    ...
    try:
        while True:
            ...
            if response and '"I"' in response and '"Q"' in response:
                ...
                if capture:
                    print(
                        f"  I/Q samples:        {len(capture.i_samples)} I, {len(capture.q_samples)} Q"
                    )
                    ...
                    print(
                        f"  Outbound readings:  {len(outbound)} (peak: {max((r.speed_mph for r in outbound), default=0):.1f} mph)"
                    )
                    print(
                        f"  Inbound readings:   {len(inbound)} (peak: {max((r.speed_mph for r in inbound), default=0):.1f} mph)"
                    )

                    if outbound_fast:
                        peak = max(r.speed_mph for r in outbound_fast)
                        print(
                            f"  SWING DETECTED:     {len(outbound_fast)} readings >= 15 mph, peak {peak:.1f} mph"
                        )
                        latencies.append(total_latency_ms)
                    else:
                        print(
                            "  NO SWING:           No outbound readings >= 15 mph (false trigger)"
                        )
                else:
            ...
            if latencies:
                ...
                print(
                    f"  Stats: {len(latencies)} captures | "
