...
def gpio_set_drive_strength(pin):
    ...
    if pin != 29:
        print(
            f"[GPIO]  Drive strength register only configured for pin 29. "
    ...
    print(f"[GPIO]  Setting pin 29 drive strength to 8mA (devmem {reg} 32 {val}) ...")
    try:
        ...
        print("[WARN]  busybox not found. Install with: sudo apt-get install busybox")
...
def enable_rolling_buffer(ser, GPIO=None, gpio_pin=None):
    ...
    if confirmed:
        print("[MODE]  Confirmed: sensor is in Rolling Buffer (Continuous Sampling Mode).")
    else:
        print("[WARN]  Rolling Buffer mode confirmation not received.")
        print('[WARN]  Expected last line: {"OperationMode":"Continuous Sampling Mode"}')
...
def read_capture(ser):
    ...
    while True:
        ...
        if now > deadline:
            elapsed = now - t_start
            print(
                f"[WARN]  Read timeout reached after {elapsed:.1f}s "
        ...
        if (now - last_data) > IDLE_TIMEOUT and total_lines > 0:
            elapsed = now - t_start
            print(
                f"[RECV]  Idle timeout — no data for {IDLE_TIMEOUT}s. "
        ...
        if ser.in_waiting:
            ...
            if t_first is None:
                t_first = time.time()
                print(
                    f"[RECV]  First data arrived {t_first - t_start:.3f}s "
                    f"after trigger. Reading ...",
                    flush=True,
                )
    ...
    print(
        f"[RECV]  Capture read complete: {total_lines} lines | "
...
def main():
    ...
    parser.add_argument(
        "--port", default=None, help="Serial port (e.g. /dev/ttyACM0). Auto-detected if omitted."
    )
    parser.add_argument(
        "--trigger",
        default="sw",
    ...
    if port is None:
        ...
        if port is None:
            print(
                "[ERROR] No USB serial port found. Ensure the sensor is plugged in and try again."
            )
            sys.exit(1)
    ...
    try:
        ...
        print(
            f"  Pre-trigger  : {pre_trig} segments "
