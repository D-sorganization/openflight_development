    ...
    def detect(self, frame: np.ndarray) -> List[Detection]:
        ...
        if circles is not None:
            ...
            for circle in circles[0, :]:
                ...
                detections.append(
                    Detection(x=float(x), y=float(y), radius=float(r), confidence=0.8)
                )
    ...
    def detect(self, frame: np.ndarray) -> List[Detection]:
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                detections.append(Detection(x=cx, y=cy, radius=radius, confidence=conf))
...
def draw_tracks(
    frame: np.ndarray, tracked: sv.Detections, track_history: dict, show_trails: bool = True
) -> np.ndarray:
...
def main():
    ...
    parser.add_argument(
        "--num-frames", type=int, default=30, help="Number of frames in headless mode"
    )
    ...
    # Detector settings
    parser.add_argument(
        "--detector", type=str, default="hough", choices=["hough", "yolo"], help="Detection method"
    )
    parser.add_argument(
        "--model",
        type=str,
        default="models/golf_ball_yolo11n_new_256.onnx",
    ...
    parser.add_argument(
        "--param2", type=int, default=27, help="Hough sensitivity (lower=more sensitive)"
    )
    ...
    parser.add_argument(
        "--min-iou", type=float, default=0.1, help="Minimum IoU threshold for matching"
    )

    # Display settings
    parser.add_argument("--no-trails", action="store_true", help="Don't draw tracking trails")
    parser.add_argument(
        "--output-dir", type=str, default=".", help="Output directory for saved frames"
    )
    ...
    if args.detector == "yolo":
        ...
        detector = HoughDetector(
            min_radius=args.min_radius, max_radius=args.max_radius, param2=args.param2
        )
    ...
    def process_frame(frame: np.ndarray, frame_num: int) -> Tuple[np.ndarray, int]:
        ...
        if detections:
            ...
            sv_detections = sv.Detections(
                xyxy=xyxy, confidence=confidence, class_id=np.zeros(len(detections), dtype=int)
            )
        ...
        cv2.putText(
            annotated,
            f"Unique IDs: {len(unique_tracks)}",
            (10, 100),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            (255, 0, 255),
