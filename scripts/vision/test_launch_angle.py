...
def run_tune_mode(camera, args, width, height):
    ...
    while True:
        ...
        n = len(detections)
        status = (
            f"p1={p1} p2={p2} r={min_r}-{max_r} dist={min_d} | "
    ...
    print("  uv run python scripts/test_launch_angle.py \\")
        ...
        while not triggered and (time.time() - start) < timeout:
            ...
            if not headless:
                ...
                if fps_count == 1:  # Once per second
                    ...
                    remaining = max(0, timeout - (time.time() - start))
                    print(
                        f"  [preview] {det_str} | fps={current_fps:.0f} | {remaining:.0f}s remaining"
                    )
    finally:
        button.close()
...
def main():
    ...
    parser.add_argument(
        "--tune", action="store_true", help="Live tuning mode — adjust Hough params with sliders"
    )
    parser.add_argument(
        "--mock", action="store_true", help="Simulate a shot sequence (no hardware needed)"
    )
    parser.add_argument(
        "--headless", action="store_true", help="No display window (for SSH sessions)"
    )
    parser.add_argument(
        "--shots", type=int, default=1, help="Number of shots to capture (default: 1)"
    )
    parser.add_argument(
        "--post-frames",
        type=int,
    ...
    parser.add_argument(
        "--gpio-pin", type=int, default=17, help="BCM GPIO pin for SEN-14262 GATE (default: 17)"
    )
    parser.add_argument(
        "--timeout", type=float, default=60.0, help="Seconds to wait per shot (default: 60)"
    )
    parser.add_argument(
        "--hough-param2",
        type=int,
    ...
    print(
        f"  Detector: Hough circles (param1={args.hough_param1}, param2={args.hough_param2}, "
    ...
    try:
        for shot_num in range(1, args.shots + 1):
            ...
            if angle:
                ...
                print(
                    f"  No launch angle (need {tracker.MIN_POSITIONS_FOR_ANGLE}+ positions, "
    ...
    if results:
        ...
        for i, a in enumerate(results, 1):
            print(
                f"  Shot {i}: V={a.vertical:+.1f}  H={a.horizontal:+.1f}  "
