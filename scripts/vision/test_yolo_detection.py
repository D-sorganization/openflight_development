...
def main():
    ...
    parser.add_argument(
        "--num-frames", type=int, default=10, help="Number of frames to capture in headless mode"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="IoU threshold for NMS (lower = fewer overlapping boxes)",
    )
    parser.add_argument("--max-det", type=int, default=1, help="Maximum detections per frame")
    # Performance options
    parser.add_argument(
        "--imgsz", type=int, default=256, help="Inference input size (256/320/640, smaller=faster)"
    )
    parser.add_argument("--half", action="store_true", help="Use FP16 half-precision inference")
    parser.add_argument(
        "--threaded", action="store_true", help="Use separate threads for capture/inference"
    )
    parser.add_argument(
        "--no-display", action="store_true", help="Skip drawing overlays for max speed"
    )
    parser.add_argument(
        "--buffer-count", type=int, default=2, help="Camera buffer count (lower=less latency)"
    )
    ...
    parser.add_argument(
        "--export-openvino", action="store_true", help="Export model to OpenVINO (supports INT8)"
    )
    ...
    if args.image:
        ...
        for d in detections:
            ...
            if (
                d["class_id"] == SPORTS_BALL_CLASS
                or d["class_id"] == GOLF_BALL_CLASS
                or "ball" in d["class"].lower()
            ):
                print("    ^^^ BALL DETECTED!")
            ...
            for r in results:
                ...
                for box in boxes:
                    ...
                    color = (
                        ...
                        or cls == GOLF_BALL_CLASS
                        or "ball" in class_name.lower()
                        else (255, 0, 0)
                    )
            ...
            while True:
                ...
                if not args.no_display:
                    ...
                    cv2.putText(
                        ...
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.7,
                        (0, 255, 0) if ball_count > 0 else (128, 128, 128),
                        2,
                    )
