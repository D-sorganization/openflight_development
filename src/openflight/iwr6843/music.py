"""Dependency-light radar-DSP primitives for the IWR6843 virtual array.
...
def _grid_steer(m):
    if m not in _STEER_CACHE:
        _STEER_CACHE[m] = np.exp(1j * np.pi * np.sin(GRID)[None, :] * np.arange(m)[:, None])
...
def est_music_fbss(x, noise_var):
    ...
    interior = np.where((spec[1:-1] > spec[:-2]) & (spec[1:-1] > spec[2:]))[0] + 1
    if len(interior) == 0:
...
def est_music_fbss_high(x, noise_var):
    """FBSS-MUSIC picking the HIGHER-angle peak when two sources resolve.

    The floor image always arrives below the direct path, so under ground
    multipath the higher peak is the ball. Call with the snapshot already in
    physical orientation (+angle = up). Hardware-validated 2026-07-12/13.
    """
    n = len(x)
    m = n // 2 + 1
    p = n - m + 1
