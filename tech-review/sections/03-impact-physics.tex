...
\begin{tikzpicture}[scale=1.05,>=Stealth]
  ...
  \draw[very thick,red!70!black,->] (0,0) -- ({8.6*cos(-2.6)},{8.6*sin(-2.6)})
    node[right] {\small launch $\approx 0.85\varphi_f + 0.15\varphi_p$};
  % angle arcs
...
w_f \approx
\begin{cases}
  0.85\text{--}0.87 & \text{driver} \\
  0.75 & \text{short irons / wedges.}
\end{cases}
\end{equation}
The weight is loft- and friction-dependent: more loft means a more oblique
impact, hence a larger tangential (friction-driven) momentum component
along the path direction. Peer-reviewed impact modeling with normal COR
plus tangential compliance reproduces the 75--85\% face dominance from
first principles~\cite{mdpifriction,facepathstudy}.
