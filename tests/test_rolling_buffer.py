...
class TestDynamicMidpointFallback:
    def test_fallback_uses_sample_rate_and_sample_count(self):
        ...
        standard_timeline = SpeedTimeline(
            readings=[
                SpeedReading(
                    speed_mph=100.0, timestamp_ms=10.0, direction="outbound", magnitude=100.0
                ),
                SpeedReading(
                    speed_mph=100.0, timestamp_ms=20.0, direction="outbound", magnitude=100.0
                ),
            ],
            sample_rate_hz=937.5,
        )
        # Outbound empty in overlapping timeline
        empty_overlapping_timeline = SpeedTimeline(
            readings=[
                SpeedReading(
                    speed_mph=100.0, timestamp_ms=10.0, direction="inbound", magnitude=100.0
                )
            ],
            sample_rate_hz=937.5,
        )

        processor.process_standard = MagicMock(return_value=standard_timeline)
        processor.process_overlapping = MagicMock(return_value=empty_overlapping_timeline)
        processor.detect_spin_multitaper = MagicMock(
            return_value=MagicMock(spin_rpm=0.0, snr=0.0, quality="none")
        )

        processed = processor.process_capture(capture)
        assert processed is not None
        expected_midpoint = (4096 / 30000.0) * 1000.0 / 2.0
        assert processed.ball_timestamp_ms == pytest.approx(expected_midpoint)
